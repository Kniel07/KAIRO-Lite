...
Generated: after Phase 3 (Backend Services, Document 13 §20 Amendment 18) + the pre-Phase-4 Component→Service hardening patch
Revision: 3.1 — Revision 3 was a full regeneration (audit only, no code changed); this is a targeted patch updating only the rows affected by the one code change the audit's findings led to (Doc 7 §8, §17, §18 below) — not a full regeneration
...
Revision 3 modified no code — every row reflected the codebase as committed at the end of Phase 3 (`4899eeb`). Revision 3.1 follows one infrastructure-only ESLint change (`eslint.config.js`) made in direct response to Revision 3's own findings; every enforcement claim below was re-verified live (a real ESLint violation written and confirmed to fail, then removed, then the full suite re-confirmed clean), not carried forward by assertion.
...
| §8 | `Component → Route → Service → Repository → Prisma`, no shortcuts | Component→Prisma lint-blocked (unchanged). Service→Prisma lint-blocked (unchanged). **Component/page→Service is now also lint-blocked** (pre-Phase-4 hardening patch, closing the gap Revision 3 found): `components/**` and `app/**` are blocked from `@/features/*/services/**`, with `app/api/**` explicitly exempt since Route Handlers are exactly where a Service call belongs. Verified live this audit: a deliberate violation in `app/__boundary_check/page.tsx` and `components/__boundary_check/Widget.tsx` both failed lint with the expected message; the identical import inside `app/api/__boundary_check/route.ts` lint-passed clean; all three scratch files were then deleted and the repository re-confirmed clean (`tsc`, `eslint`, `prettier`, 78 tests all pass). Route→Repository: `N/A`, no Route Handler exists yet | `eslint.config.js` | `LINT-TIME` (Component→Prisma, Service→Prisma, Component/page→Service) / `N/A` (Route→Repository — no Route Handler exists yet to test) |
...
**One finding from Revision 3, closed by the pre-Phase-4 hardening patch:** nothing blocked a future `app/**` or `components/**` file from importing a Service directly, skipping the Route Handler layer Document 7 §8's chain specifies. `eslint.config.js` now blocks `@/features/*/services/**` from `components/**` and `app/**`, exempting `app/api/**` — see the Doc 7 §8 row above for the live-verification detail (violation written and confirmed to fail, Route Handler exemption confirmed to still pass, scratch files deleted, full suite re-confirmed clean). No design question remained to defer: Route Handlers are the only place this chain permits a Service call, so the exemption is exactly `app/api/**`, nothing narrower or broader.

---

# 18. Summary — Rules Still Enforced Only By Convention

Updated for the pre-Phase-4 hardening patch — the Component→Service item Revision 3 found is now closed and removed from this list.

1. **Naming conventions** (Doc 6, most of it) — no automated casing/suffix checks. Includes a *specific* instance: nothing lint-enforces "class name ends in `Service`" even though all six current ones comply.
...
3. **No `ts-ignore` without justification, import ordering, catch-block error handling** (Doc 7 §3, §10, §16) — unchanged, still unchecked.
4. **Phase-gating itself** (Doc 9) — unchanged, 100% `PROCESS`, and per the prior revision's note, intentionally so.
5. **Raw SQL soft-delete documentation** (Doc 7 §8, amended) — unchanged, `MANUAL REVIEW` by design (a documentation requirement, not a technical one).
6. **"Business logic only in Services" as a negative constraint** (Doc 7 §7) — the positive claim (Services *do* own the logic) is `RUNTIME`-demonstrated; the negative claim (nothing else *could*) remains convention-dependent until Route Handlers/Components exist to test against.
